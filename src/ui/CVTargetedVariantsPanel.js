...
function stateLabel(state) {
  return state === 'evidence_found' ? 'Already supported by your CV' : state === 'keyword_without_evidence' ? 'Mentioned, but needs a clear example' : 'Not found in your CV';
}

function renderAnalysis(analysis) {
  if (!analysis) return '<p style="color:rgba(255,255,255,.58);font-size:13px">Your result will appear here. Checking does not change or save your main CV.</p>';
  return `<div class="cv-variant-analysis" style="display:grid;gap:12px">
    ...
    <p style="margin:0;color:rgba(255,255,255,.68);font-size:12px">This is a simple comparison, not a promise that you will pass an ATS system.</p>
...
export function renderCVTargetedVariantsPanel(container, { ownerUserId = 'local-dev-user', getBaseProfile } = {}) {
  ...
  let variants = [];
  container.innerHTML = `
        <details class="cv-targeted-variants cv-job-tailor" style="margin-top:24px;background:rgba(6,182,212,.06);border:1px solid rgba(6,182,212,.24);border-radius:16px;padding:18px;color:#fff">
      <summary class="cv-job-tailor-summary"><span><small>OPTIONAL PRO TOOL</small><strong>Prepare a CV copy for a specific job</strong><em>Compare a job description with your CV. Your main CV will not change.</em></span><b>Open tool</b></summary>
      <div class="cv-job-tailor-content"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div><h2 style="margin:5px 0;font-size:20px">Prepare a job-specific copy</h2><p style="margin:0;color:rgba(255,255,255,.68);font-size:12px;max-width:680px">1. Enter the job title. 2. Paste the job description. 3. Check what your CV already supports, then save a separate private copy if you want.</p></div><a href="/cv?profile=${encodeURIComponent(initialProfile.id)}" style="color:#67e8f9;font-size:12px;text-decoration:none">← Back to main CV</a></div>
      <div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:14px;margin-top:16px">
        <form id="cv-targeted-form" style="display:grid;gap:10px"><label style="font-size:12px">1. Job title<input name="role" required maxlength="160" placeholder="e.g. Frontend Developer" style="display:block;width:100%;box-sizing:border-box;margin-top:4px;padding:9px;border-radius:8px;border:1px solid rgba(255,255,255,.15);background:#090b12;color:#fff"></label><label style="font-size:12px">Company name <small>(optional and private)</small><input name="company" maxlength="160" placeholder="Optional" style="display:block;width:100%;box-sizing:border-box;margin-top:4px;padding:9px;border-radius:8px;border:1px solid rgba(255,255,255,.15);background:#090b12;color:#fff"></label><label style="font-size:12px">2. Job description<textarea name="jobDescription" required minlength="15" rows="9" placeholder="Copy and paste the job requirements here." style="display:block;width:100%;box-sizing:border-box;margin-top:4px;padding:9px;border-radius:8px;border:1px solid rgba(255,255,255,.15);background:#090b12;color:#fff;resize:vertical"></textarea></label><label style="font-size:12px">New summary <small>(optional)</small><input name="summary" maxlength="2400" placeholder="Leave empty to keep your current summary" style="display:block;width:100%;box-sizing:border-box;margin-top:4px;padding:9px;border-radius:8px;border:1px solid rgba(255,255,255,.15);background:#090b12;color:#fff"></label><label style="font-size:11px;color:rgba(255,255,255,.7)"><input type="checkbox" name="confirmSummary"> I wrote this new summary and confirm it is true.</label><div style="display:flex;gap:8px;flex-wrap:wrap"><button type="button" data-variant-analyze style="background:#0891b2;color:#fff;border:0;border-radius:8px;padding:9px 13px;font-weight:700">3. Compare with my CV</button><button type="submit" data-variant-create disabled style="background:#7c3aed;color:#fff;border:0;border-radius:8px;padding:9px 13px;font-weight:700">Save a separate private copy</button></div><p data-variant-status aria-live="polite" style="min-height:16px;margin:0;font-size:12px"></p></form>
