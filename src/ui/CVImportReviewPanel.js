...
export function renderCVImportReviewPanel(container, { ownerUserId = 'local-dev-user', getBaseProfile, onSaved, autoOpen = false } = {}) {
  ...
  function renderImportDialog() {
    ...
    function fieldRow(path, item, multiline = false) {
      ...
      return `<label class="cv-import-field"><input type="checkbox" data-import-select="${escape(path)}" ${item.selected ? 'checked' : ''}><span><strong>${escape(path.split('.').slice(-1)[0])}</strong><em>EXTRACTED · REVIEW</em>${input}</span></label>`;
    }
    ...
    function renderReview(review) {
      ...
      view.innerHTML = `<div style="background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.25);border-radius:10px;padding:10px;color:#fde68a;font-size:12px"><strong>Review required before saving.</strong>${review.warnings.length ? `<ul style="margin:6px 0 0;padding-left:18px">${review.warnings.map(w => `<li>${escape(w)}</li>`).join('')}</ul>` : ''}</div><div class="cv-import-section"><h3>Contact</h3>${Object.entries(selection.contact).map(([key, item]) => fieldRow(`contact.${key}`, item)).join('')}</div><div class="cv-import-section"><h3>Summary</h3>${fieldRow('summary', selection.summary, true)}</div>${['experience','education','skills','projects','certifications','languages','training','activities'].map(key => `<div class="cv-import-section"><h3>${escape(key)}</h3>${listRows(key, selection[key] || [])}</div>`).join('')}<label style="display:flex;gap:8px;align-items:center;font-size:12px;color:rgba(255,255,255,.75);margin-top:12px"><input type="checkbox" data-import-overwrite> Allow replacing existing fields (otherwise empty-only/merge is used)</label><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:14px"><button type="button" data-import-save style="background:#7c3aed;color:#fff;border:0;border-radius:8px;padding:10px 14px;font-weight:800">Save selected fields</button><button type="button" data-import-cancel style="background:rgba(255,255,255,.08);color:#fff;border:1px solid rgba(255,255,255,.16);border-radius:8px;padding:10px 14px">Cancel</button></div>`;
      ...
      view.querySelector('[data-import-save]').addEventListener('click', () => {
        ...
        view.querySelectorAll('[data-import-field]').forEach(input => { const path = input.dataset.importField.split('.'); const key = path[0]; if (path.length === 2 && /^\d+$/.test(path[1])) nextSelection[key][Number(path[1])].value = input.value; else if (key === 'contact') nextSelection.contact[path[1]].value = input.value; else nextSelection[key].value = input.value; });
